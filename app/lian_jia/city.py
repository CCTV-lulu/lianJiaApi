...
class District(AlchemyMixin, Base):
    __tablename__ = 'districts' #区域的数据库

    id = Column(types.Integer, primary_key=True)
    city_id = Column(types.Integer, ForeignKey(City.id), nullable=False)
    name = Column(types.String(32), nullable=False)
    quan_pin = Column(types.String(100), nullable=False) #商圈的拼音名称
    biz_circles_count = Column(types.Integer, nullable=False) #小区数？貌似不是这样
